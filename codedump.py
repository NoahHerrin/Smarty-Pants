# code used for testing downloaded graph data
...
#        'weight': 5
#    }
#    db2.update_document(identifier, new_data)
...
#  ~ figure out how to store nodes and verticies DONE
#  ~ figure out how to read stored node and vertex data DONE
#  ~ form graph from data from the Database DONE
#  ~ run dijkstra's shortest path algorithm on database graph DONE
#  ~ build an interface to interact with program using flask
...
graph.getVertex("A").printData()
graph2.calculateShortestPath(graph2.getVertex("A"))
graph2.getShortestPathTo(graph2.getVertex("E"))
